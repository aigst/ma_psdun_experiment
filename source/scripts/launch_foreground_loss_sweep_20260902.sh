...
OUT="$ROOT/new_results_20260902_foreground_loss"
mkdir -p "$OUT"

run_one() {
  local gpu="$1" name="$2" fw="$3" seed="$4"
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    nohup python3 "$SRC/new_sample_train.py" \
      ...
      > "$OUT/$name.log" 2>&1 &
  echo "$name pid=$! gpu=$gpu foreground=$fw seed=$seed"
}

run_one 0 fg000 0.0 20260990
run_one 1 fg050 0.5 20260991
run_one 2 fg100 1.0 20260992
run_one 3 fg200 2.0 20260993
wait
