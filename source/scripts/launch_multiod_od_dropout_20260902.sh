...
OUT="$ROOT/new_results_20260902_od_dropout"
mkdir -p "$OUT"

run_one() {
  local gpu="$1" name="$2" dropout="$3" seed="$4"
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    nohup python3 "$SRC/new_sample_train.py" \
      ...
      > "$OUT/$name.log" 2>&1 &
  echo "$name pid=$! gpu=$gpu dropout=$dropout seed=$seed"
}

run_one 0 od_drop000 0.00 20260950
run_one 1 od_drop010 0.10 20260951
run_one 2 od_drop025 0.25 20260952
run_one 3 od_drop050 0.50 20260953
wait
