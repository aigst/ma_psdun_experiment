...
OUT=${MA_PSDUN_PSF_OUT:-$ROOT/results_psf_sigma_cv_20260904}
...
summary = {
    ...
    "psf_sigma": sigma,
    "selection_ranking": [{
        "name": f"mean_detrend_w127_psf{sigma:g}",
...
root = Path(sys.argv[1])
rows = []
for path in sorted(root.glob("sigma_*/selection/ranking.json")):
    payload = json.loads(path.read_text())
    row = payload["selection_ranking"][0].copy()
    row["psf_sigma"] = payload["psf_sigma"]
    rows.append(row)
rows.sort(key=lambda row: row["cross_validation_ssim"], reverse=True)
summary = {
    "status": "completed",
    "ranking": rows,
    "object_level_disjoint": True,
...
(root / "ranking.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n")
(root / "SELECTION_DONE").write_text("completed\n")
print(json.dumps(summary, ensure_ascii=False))
PY
