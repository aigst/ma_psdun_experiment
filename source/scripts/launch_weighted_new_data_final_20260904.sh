...
OUT=${MA_PSDUN_WEIGHTED_FINAL_OUT:-$ROOT/results_weighted_new_20260904}
...
if [[ ! -f "$OUT/SELECTION_DONE" || ! -f "$OUT/selection/ranking.json" ]]; then
  echo "weighted selection is incomplete: $OUT" >&2
  exit 1
fi
...
common=(
  ...
  --new-objects obj22-20260902,obj24-20260903,obj25-20260903
...
run_final() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    "$PYTHON" "$SRC/weighted_new_data_train.py" --mode final \
      --exp-dir "$OUT/final/seed$seed" --train-objects "$ALL" --seed "$seed" \
      "${common[@]}" > "$OUT/final/seed$seed.log" 2>&1 &
}

run_final 0 20261171
run_final 1 20261172
run_final 2 20261173
run_final 3 20261174
wait

members=$OUT/final/seed20261171,$OUT/final/seed20261172,$OUT/final/seed20261173,$OUT/final/seed20261174
CUDA_VISIBLE_DEVICES="" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
  "$PYTHON" "$SRC/weighted_new_data_train.py" --mode ensemble \
    --exp-dir "$OUT/final/ensemble" --members "$members"

selection_dirs=$OUT/selection/fold1/w100,$OUT/selection/fold2/w100,$OUT/selection/fold3/w100,$OUT/selection/fold4/w100
...
summary = {
    "status": "completed",
    "selected_new_object_weight": 1.0,
...
(root / "weighted_final_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n")
(root / "WEIGHTED_FINAL_DONE").write_text("completed\n")
